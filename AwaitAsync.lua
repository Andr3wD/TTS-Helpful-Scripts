...
local Promise = (function()
    ...
    local function fire(deferred)
        ...
        promise(deferred, next, function()
            ...
            if deferred.value == deferred then
                ...
                promise(deferred, next,
                        function() finish(deferred, RESOLVED) end,
                        function(state) finish(deferred, state) end, function()
    ...
    function M.first(args)
        ...
        for _, v in ipairs(args) do
            v:next(function(res) d:resolve(res) end,
...
local function resume(co, ...)
    ...
    if table.remove(response, 1) then
        if coroutine.status(co) == 'dead' then
            coroPromises[co]:resolve(table.unpack(response))
        end
    else
        coroPromises[co]:reject(table.unpack(response))
    end
end
